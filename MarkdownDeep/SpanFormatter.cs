...
namespace MarkdownDeep
{
	public class SpanFormatter : StringScanner
	{
		...
		public void FormatParagraph(StringBuilder dest, string str, int start, int len)
		{
		...
		public void Format(StringBuilder dest, string str)
		{
			Format(dest, str, 0, str.Length);
		}

		// Format a range in an input string and write it to the destination string builder.
		public void Format(StringBuilder dest, string str, int start, int len)
		{
			// Parse the string into a list of tokens
			Tokenize(str, start, len);
		...
		public void FormatPlain(StringBuilder dest, string str, int start, int len, Action<Block,Token,string> customProcess)
		{
			...
			// Render all tokens
			RenderPlain(dest, str, customProcess);
		}
		...
		public string MakeID(string str, int start, int len)
		{
			// Parse the string into a list of tokens
			Tokenize(str, start, len);
		...
		public Token CreateToken(TokenType type, int startOffset, int length)
		{
		...
		// CreateToken - create or re-use a token object
		public Token CreateToken(TokenType type, object data)
		{
			if (m_SpareTokens.Count != 0)
			{
				var t = m_SpareTokens.Pop();
