...
namespace MarkdownDeep
{
	...
	public class Block
	{
		...
		public void RenderChildren(Markdown m, StringBuilder b)
		{
		...
		public void RenderChildrenPlain(Markdown m, StringBuilder b, Action<Block,Token,string> customProcess)
		{
			foreach (var block in children)
			{
				block.RenderPlain(m, b, customProcess);
			}
		}

		public string ResolveHeaderID(Markdown m)
		{
			// Already resolved?
		...
		public void RenderPlain(Markdown m, StringBuilder b, Action<Block,Token,string> customProcess)
		{
			...
			switch (blockType)
			{
				...
				case BlockType.p:
				case BlockType.span:
					m.SpanFormatter.FormatPlain(b, buf, contentStart, contentLen, customProcess);
					b.Append(" ");
					break;
				...
				case BlockType.dt:
					{
						if (children == null)
						{
							...
							{
								var str = l.Trim();
								m.SpanFormatter.FormatPlain(b, str, 0, str.Length, customProcess);
							}
						}
						else
						{
		...
		public int lineStart;
		public int lineLen;
		public object data;			// content depends on block type
